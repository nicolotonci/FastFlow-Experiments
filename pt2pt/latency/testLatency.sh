...
for ((messageSize=2; messageSize<=1048576; messageSize*=2)); do
    for((ripetizione=0; ripetizione<$RIPETIZIONI; ripetizione+=1)); do
        if [ "$USE_SLURM" -eq 1 ]; then
            srun -N 2 --exclusive --cpus-per-task=36 --export=ALL,UCX_ZCOPY_THRESH=2M $(pwd)/LatencyFF  $messageSize --DFF_Config=$(pwd)/dff.json | tail -1
        else
...
for ((messageSize=2; messageSize<=1048576; messageSize*=2)); do
    for((ripetizione=0; ripetizione<$RIPETIZIONI; ripetizione+=1)); do
       if [ "$USE_SLURM" -eq 1 ]; then
            srun -N 2 --exclusive --cpus-per-task=36 --export=ALL,UCX_ZCOPY_THRESH=2M $(pwd)/LatencyMTCL $messageSize | tail -1
        else
...
for ((messageSize=2; messageSize<=1048576; messageSize*=2)); do
    for((ripetizione=0; ripetizione<$RIPETIZIONI; ripetizione+=1)); do
        if [ "$USE_SLURM" -eq 1 ]; then
            srun -N 2 --exclusive --cpus-per-task=36 --export=ALL,UCX_ZCOPY_THRESH=2M $(pwd)/LatencyMPI $messageSize | tail -1
        else
