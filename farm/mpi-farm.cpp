...
void Collector(int num_workers) {
    ...
    char* buffers[2] = {new char[task_size * batch_tasks], new char[task_size * batch_tasks]};

    MPI_Request requests[2];

    error = MPI_Irecv(buffers[0], buff_size, MPI_BYTE, MPI_ANY_SOURCE, RESULT_TAG, MPI_COMM_WORLD, requests);
    CHECK_ERROR(error);
    error = MPI_Irecv(buffers[1], buff_size, MPI_BYTE, MPI_ANY_SOURCE, RESULT_TAG, MPI_COMM_WORLD, requests+1);
    CHECK_ERROR(error);
